...
{
    public class AgentActivityUpdateViewModelValidator : BaseValidation<AgentActivityUpdateViewModel>
    {
        private readonly ICSFEApiClient _csfeApiClient;
        private IEnumerable<Location> locations;
        private Event relatedEvent;

        public AgentActivityUpdateViewModelValidator(ICSFEApiClient csfeApiClient)
        {
            ...
            RuleFor(a => a.ActivityDate)
                .NotEmpty()
                .When(a => a.IsPropertyDirty(nameof(AgentActivityUpdateViewModel.ActivityDate)));
        ...
        private async Task<bool> IsLocationRequiredAsync(AgentActivityUpdateViewModel viewModel)
        {
            ...
            if (!viewModel.IsPropertyDirty(nameof(AgentActivityUpdateViewModel.Location)))
            {
                return false;
            }
        ...
        private async Task<bool> IsRemarkRequiredAsync(AgentActivityUpdateViewModel viewModel)
        {
            if (string.IsNullOrEmpty(viewModel.ActivityCode))
            {
                return false;
            ...
            if (!viewModel.IsPropertyDirty(nameof(AgentActivityUpdateViewModel.Remark)))
            {
                return false;
            }
