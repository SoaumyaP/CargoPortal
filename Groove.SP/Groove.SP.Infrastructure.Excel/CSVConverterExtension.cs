...
namespace Groove.SP.Infrastructure.Excel
{
    ...
    {
        /// <summary>
        /// Note: EPPlus does not work with the XLS format.
        /// </summary>
        ...
        public static byte[] ConvertToCsv(this MemoryStream stream, string excelFilePath)
        {
            using (var sr = new StreamReader(stream))
            {
                ...
                while (row_no < ds.Tables[0].Rows.Count)
                {
                    ...
                    {
                        var cell = ds.Tables[0].Rows[row_no][i];
                        if (cell == null)
                        {
                            // add a cell value for empty cells to keep data aligned.
                            AddCellValue(string.Empty, arr);
                        }
                        else
                        {
                            AddCellValue(cell.ToString(), arr);
                        }
                    }
                    row_no++;
                    csvContent += string.Join(",", arr) + "\n";
                }
                return System.Text.Encoding.ASCII.GetBytes(csvContent);
            }
        }
        ...
        private static string ToDelimitedString(this List<string> list, string delimiter = ":", bool insertSpaces = false, string qualifier = "", bool duplicateTicksForSQL = false)
        {
