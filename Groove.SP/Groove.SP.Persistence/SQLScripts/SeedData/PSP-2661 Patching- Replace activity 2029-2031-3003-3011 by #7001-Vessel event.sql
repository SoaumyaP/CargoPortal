...
-- Description:		[PSP-2661] - Replace departure activities (event #2029, 3003, 2031, 3011) by Vessel activity (event #7001)
-- =============================================
SET NOCOUNT ON;
...
WHERE ActivityCode IN (
	'2029', -- SM - Shipment actual departure from origin port
	'3003', -- CM - Container - Actual Port Departure
	'2031', -- SA - Transhipment port actual departure
	'3011' -- CA - Discharge at Port of Tran-shipment (Z4)
)
...
FROM ConsignmentItineraries csmi
	...
	INNER JOIN @TShipmentActivity sa ON csmi.ShipmentId = sa.Id AND sa.[Location] = fs.LocationFromName
...
FROM ContainerItineraries ctni
	...
	INNER JOIN FreightSchedulers fs ON fs.Id = i.ScheduleId
	INNER JOIN @TContainerActivity ca ON ctni.ContainerId = ca.Id AND ca.[Location] = fs.LocationFromName
...
SELECT
	t.CreatedDate,
	t.CreatedBy,
	'7001' as [ActivityCode],
	'VA' as [ActivityType],
	'Vessel Departure' as [ActivityDescription],
