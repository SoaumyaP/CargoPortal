...
IF OBJECT_ID('spu_GetCustomerPurchaseOrderList_ExternalUsers', 'P') IS NOT NULL
DROP PROC dbo.spu_GetCustomerPurchaseOrderList_ExternalUsers
GO
...
CREATE PROCEDURE spu_GetCustomerPurchaseOrderList_ExternalUsers
	...
	@CurrentUserOrganizationId BIGINT = 0
AS
BEGIN
	...
	-- Parse affiliates to table, then compare to supplier or delegation of PO
	DECLARE @AffiliatesTable TABLE (
		[Id] BIGINT
	)
	...
	INSERT INTO @AffiliatesTable
	SELECT CAST(tmp.[Value] AS BIGINT)
			...
			FROM
				(
					...
					WHERE
						 ...
						 (
							SELECT 1
							FROM [PurchaseOrderContacts] POCP  WITH (NOLOCK)
							WHERE 
								PO.Id = POCP.PurchaseOrderId
								AND POCP.OrganizationId = @CustomerOrganizationId 
								AND POCP.OrganizationRole = 'Principal'
						 )
						 AND EXISTS
						 (
							SELECT 1
							FROM [PurchaseOrderContacts] POCSD  WITH (NOLOCK)
							WHERE 
								PO.Id = POCSD.PurchaseOrderId
								AND 
								(
									(POCSD.OrganizationId = @SupplierOrganizationId AND  POCSD.OrganizationRole = 'Supplier' AND POCSD.OrganizationId = @CurrentUserOrganizationId)
								    OR
									(POCSD.OrganizationId IN (SELECT Id FROM @AffiliatesTable) AND POCSD.OrganizationRole = 'Delegation')
									OR
									(POCSD.OrganizationId IN (SELECT Id FROM @AffiliatesTable) AND POCSD.OrganizationRole = 'Origin Agent')
									OR
									(POCSD.OrganizationId IN (SELECT Id FROM @AffiliatesTable) AND POCSD.OrganizationRole = 'Destination Agent')
							    )
						 )
						AND PO.[Status] = 1
	...
	BEGIN
		...
		CROSS APPLY
		(
			SELECT TOP(1) sc.OrganizationId AS SupplierId, sc.CompanyName AS SupplierName
			FROM PurchaseOrderContacts sc WITH (NOLOCK)
			WHERE t1.Id = sc.PurchaseOrderId AND sc.OrganizationRole = 'Supplier'
		) t2
		OUTER APPLY
		...
		WHERE (@SupplierOrganizationId = 0 OR @SupplierOrganizationId = t2.SupplierId)
		AND (@SupplierOrganizationId <> 0 OR @SupplierCompanyName = '' OR @SupplierOrganizationId = t2.SupplierName)
		AND (@SupplierOrganizationId <> 0 OR @SupplierCompanyName <> '' OR (
							t3.DelegationId IN (SELECT Id FROM @AffiliatesTable)
							OR t4.OriginAgentId IN (SELECT Id FROM @AffiliatesTable)
							OR t5.DestinationAgentId IN (SELECT Id FROM @AffiliatesTable))
		)
		SET @RowCount = @@ROWCOUNT
	END
