...
-- Description: Search shipment on link to shipment popup for external user
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[spu_GetShipmentSelection_HouseBL_External]
...
BEGIN
	...
	WHERE 
		...
		(
			...
			FROM ShipmentContacts SC (NOLOCK)
			WHERE 
				SC.ShipmentId = ShipmentCTE.Id
				AND SC.OrganizationId IN (SELECT value FROM [dbo].[fn_SplitStringToTable](@affiliates,','))
		)
END
