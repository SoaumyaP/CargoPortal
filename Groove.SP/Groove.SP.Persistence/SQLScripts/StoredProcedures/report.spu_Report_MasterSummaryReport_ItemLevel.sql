...
IF OBJECT_ID('report.spu_MasterSummaryReport_ItemLevel', 'P') IS NOT NULL
DROP PROC [report].[spu_MasterSummaryReport_ItemLevel]
GO

-- =============================================
-- Author:		Cuong Duong Duy
-- Create date: 03 Aug 2021
-- Description:	Get data for Master summary report (level item) run on Telerik Reporting Server
-- =============================================
CREATE PROCEDURE [report].[spu_MasterSummaryReport_ItemLevel]
	@ETDFrom DATETIME2 
	,@ETDTo DATETIME2
	,@ETAFrom DATETIME2 
	,@ETATo DATETIME2
	...
	,@Supplier BIGINT
	,@PromotionCode NVARCHAR(MAX) -- Filter data against POLineItem.ProductRemark
...
BEGIN
	...
	DECLARE @SelectedShipFromLocationId TABLE (Id BIGINT NOT NULL)
	DECLARE @SelectedShipToLocationId TABLE (Id BIGINT NOT NULL)

	IF (@ShipFrom IS NOT NULL AND @ShipFrom != '')
	BEGIN
		INSERT INTO @SelectedShipFromLocationId
	...
	BEGIN
		INSERT INTO @SelectedShipToLocationId
	...
	WHERE 
		...
		AND (NOT EXISTS(SELECT 1 FROM @SelectedShipFromLocationId) OR PO.ShipFromId IN (SELECT * FROM @SelectedShipFromLocationId))
		AND (NOT EXISTS(SELECT 1 FROM @SelectedShipToLocationId) OR PO.ShipToId IN (SELECT * FROM @SelectedShipToLocationId))
	...
	INTO #PurchaseOrder1CTE
	FROM #PurchaseOrderCTE PO
	INNER JOIN POLineItems POL WITH(NOLOCK) ON PO.Id = POL.PurchaseOrderId  AND (@PromotionCode IS NULL OR POL.ProductRemark = @PromotionCode) -- filter on promotion code
