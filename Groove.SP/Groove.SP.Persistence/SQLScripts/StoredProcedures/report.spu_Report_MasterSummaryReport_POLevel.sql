...
IF OBJECT_ID('report.spu_MasterSummaryReport_POLevel', 'P') IS NOT NULL
DROP PROC [report].spu_MasterSummaryReport_POLevel
GO

-- =============================================
-- Author:		Cuong Duong Duy
-- Create date: 03 Aug 2021
-- Description:	Get data for Master summary report (level PO) run on Telerik Reporting Server
-- =============================================
CREATE PROCEDURE [report].[spu_MasterSummaryReport_POLevel]
	@ETDFrom DATETIME2 
	,@ETDTo DATETIME2
	,@ETAFrom DATETIME2 
	,@ETATo DATETIME2
	,@ATDFrom DATETIME2
	,@ATDTo DATETIME2
	,@CargoReadyDateFrom DATETIME2
	,@CargoReadyDateTo DATETIME2
	,@ShipFromCountry NVARCHAR(MAX)
	,@ShipFrom NVARCHAR(MAX)
	,@ShipToCountry NVARCHAR(MAX)
	,@ShipTo NVARCHAR(MAX)
	,@PONoFrom NVARCHAR(MAX)
	,@PONoTo NVARCHAR(MAX)
	,@BookingNumber NVARCHAR(MAX)
	,@ShipmentNumber NVARCHAR(MAX)
	,@ContractType NVARCHAR(512)
	,@Supplier BIGINT
	,@HouseBLNumber NVARCHAR(MAX) -- House bill of lading number (BillOfLadings table)
	,@MasterBLNumber NVARCHAR(MAX) -- Master bill of lading number (MasterBills table)
	,@POStage INT
	,@MovementType NVARCHAR(10)
	,@ContainerNumber NVARCHAR(MAX)
	,@ContainerSize NVARCHAR(MAX)
	,@SelectedCustomerId BIGINT
	,@NumberOfTop BIGINT = 9223372036854775807 -- Max value of bigint
AS
BEGIN
	...
	IF(OBJECT_ID('tempdb..#TmpEquipmentTypeTbl') IS NOT NULL)
	BEGIN
		DROP TABLE #TmpEquipmentTypeTbl
	END
	CREATE TABLE #TmpEquipmentTypeTbl (Id INT NOT NULL, Code NVARCHAR(10), [Name] NVARCHAR(50) NOT NULL)

	IF(OBJECT_ID('tempdb..#TmpFilteredPOLTbl') IS NOT NULL)
	BEGIN
		DROP TABLE #TmpFilteredPOLTbl
	END
	CREATE TABLE #TmpFilteredPOLTbl (POId BIGINT, POLId BIGINT)

	IF(OBJECT_ID('tempdb..#TmpCustomerRelationshipTbl') IS NOT NULL)
	BEGIN
		DROP TABLE #TmpCustomerRelationshipTbl
	END
	CREATE TABLE #TmpCustomerRelationshipTbl (SupplierId BIGINT, CustomerRefId NVARCHAR(20))

	IF(OBJECT_ID('tempdb..#TmpSelectedShipFromLocationId') IS NOT NULL)
	BEGIN
		DROP TABLE #TmpSelectedShipFromLocationId
	END
	CREATE TABLE #TmpSelectedShipFromLocationId (Id BIGINT NOT NULL)

	IF(OBJECT_ID('tempdb..#TmpSelectedShipToLocationId') IS NOT NULL)
	BEGIN
		DROP TABLE #TmpSelectedShipToLocationId
	END
	CREATE TABLE #TmpSelectedShipToLocationId (Id BIGINT NOT NULL)
	

	IF (@ShipFrom IS NOT NULL AND @ShipFrom != '')
	BEGIN
		INSERT INTO #TmpSelectedShipFromLocationId
		SELECT [Value] AS Id FROM [dbo].[fn_SplitStringToTable](@ShipFrom, ',')
	END
	ELSE
	BEGIN
		IF (@ShipFromCountry IS NOT NULL AND @ShipFromCountry != '')
		BEGIN
			INSERT INTO #TmpSelectedShipFromLocationId
			SELECT [Value] AS Id FROM [dbo].[fn_SplitStringToTable](@ShipFromCountry, ',')
		END
	END

	IF (@ShipTo IS NOT NULL AND @ShipTo != '')
	BEGIN
		INSERT INTO #TmpSelectedShipToLocationId
		SELECT [Value] AS Id FROM [dbo].[fn_SplitStringToTable](@ShipTo, ',')
	END
	ELSE
	BEGIN
		IF (@ShipToCountry IS NOT NULL AND @ShipToCountry != '')
		BEGIN
			INSERT INTO #TmpSelectedShipToLocationId
			SELECT [Value] AS Id FROM [dbo].[fn_SplitStringToTable](@ShipToCountry, ',')
		END
	END

	-- Set values for variables
	INSERT INTO #TmpEquipmentTypeTbl
	...
	INSERT INTO #TmpCustomerRelationshipTbl
	...
	(
		...
		WHERE 
			...
			AND (@BookedPOFiltering = 0 OR EXISTS (SELECT 1 FROM #TmpFilteredPOLTbl TMP WHERE PO.Id = TMP.POId))			
			...
			AND (NOT EXISTS(SELECT 1 FROM #TmpSelectedShipFromLocationId) OR PO.ShipFromId IN (SELECT * FROM #TmpSelectedShipFromLocationId))
			AND (NOT EXISTS(SELECT 1 FROM #TmpSelectedShipToLocationId) OR PO.ShipToId IN (SELECT * FROM #TmpSelectedShipToLocationId))
	...
	(
		SELECT			
			...
			,PO.ContainerType [POContainerType] -- to link with #TmpEquipmentTypeTbl (Column 17)
		...
		WHERE 
			...
			(@BookedPOFiltering = 0 OR EXISTS (SELECT 1 FROM #TmpFilteredPOLTbl TMP WHERE POL.PurchaseOrderId = TMP.POId AND POL.Id = TMP.POLId	))
			...
			-- filter on Container size
			AND 
				(
				@ContainerSize IS NULL 
				OR EXISTS (
					SELECT 1
					FROM Containers CON WITH(NOLOCK)
					INNER JOIN ShipmentLoadDetails SHLD WITH(NOLOCK) ON CON.Id = SHLD.ContainerId
					INNER JOIN CargoDetails CD WITH(NOLOCK) ON SHLD.CargoDetailId = CD.Id 
					WHERE CD.ShipmentId = SHI.Id AND CD.OrderId = PO.Id AND CD.ItemId = POL.Id 
						AND (CON.ContainerType IN (SELECT [Value] FROM [dbo].[fn_SplitStringToTable](@ContainerSize, ','))))
				)
	)
	...
	,PurchaseOrder2CTE AS (
		...
		SELECT 
			...
			,STUFF(
						(	SELECT ', ' + ISNULL(CEQ.[Name], CON.ContainerType)
							FROM Containers CON WITH(NOLOCK)
								LEFT JOIN #TmpEquipmentTypeTbl CEQ ON CON.ContainerType = CEQ.Code
								INNER JOIN ShipmentLoadDetails SHLD WITH(NOLOCK) ON CON.Id = SHLD.ContainerId
								INNER JOIN CargoDetails CD WITH(NOLOCK) ON SHLD.CargoDetailId = CD.Id  
								WHERE CD.ShipmentId = CTE.ShipmentId AND CD.OrderId = CTE.POId AND CD.ItemId = CTE.POLineItemId
							FOR XMl PATH('') 
		...
		LEFT JOIN #TmpCustomerRelationshipTbl CR ON CR.SupplierId = CTE.SupplierId
		...
		WHERE
			...
			AND (
				...
				OR EXISTS (
					SELECT 1
					FROM ContractMaster CM WITH(NOLOCK)
					WHERE CM.CarrierContractNo = CTE.ShipmentCarrierContractNo AND CM.ContractType = @ContractType
				)
			)
	)
	...
	, DataReturnCTE AS (
		SELECT
			...
			, STUFF (
				(	SELECT ', ' + POFFLEQP.[Name]
					...
					INNER JOIN #TmpEquipmentTypeTbl POFFLEQP ON POFFL.POFulfillmentId = PO.POFFId AND POFFL.EquipmentType = POFFLEQP.Id
					FOR XMl PATH('') 
				), 1, 2, ''
		...
		LEFT JOIN #TmpEquipmentTypeTbl POEQP ON PO.POContainerType = POEQP.Id
	...
	SELECT
		...
		,STUFF (
			(	SELECT DISTINCT ', ' + [Actual Shipped Container Number]
				FROM DataReturnCTE TMP
				WHERE TMP.PO# = DataReturnCTE.PO#
				FOR XMl PATH('') 
			), 1, 2, ''
		) AS [Actual Shipped Container Number] --Column 41
		,STUFF (
	...
	OFFSET 0 ROWS FETCH NEXT @NumberOfTop ROWS ONLY
