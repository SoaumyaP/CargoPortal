...
IF EXISTS(SELECT * FROM sys.views WHERE NAME = 'vw_ShipmentList_ShipmentVolumeByOriginDestination_ExternalUsers')
BEGIN
	 DROP VIEW vw_ShipmentList_ShipmentVolumeByOriginDestination_ExternalUsers;
END
GO

CREATE VIEW [dbo].[vw_ShipmentList_ShipmentVolumeByOriginDestination_ExternalUsers]
AS
	...
	SELECT 
			...
			T1.Shipper,
			T2.Consignee,
			SC.OrganizationId -- to filter with affiliate
			,T4.*
