...
IF EXISTS(SELECT * FROM sys.views WHERE NAME = 'vw_PurchaseOrderList_Booked_ExternalUsers')
BEGIN
	 DROP VIEW vw_PurchaseOrderList_Booked_ExternalUsers;
END
GO

CREATE VIEW [dbo].[vw_PurchaseOrderList_Booked_ExternalUsers]
AS
				...
				EXISTS (
					...
					WHERE POFFO.PurchaseOrderId = PO.Id
						AND NOT EXISTS (
								...
								INNER JOIN GlobalIdActivities GACT WITH(NOLOCK) ON GACT.GlobalId = CONCAT('SHI_', SHI.Id)
								INNER JOIN Activities ACT WITH(NOLOCK) ON ACT.Id = GACT.ActivityId
								WHERE ACT.ActivityCode = '2014' AND ACT.ActivityType = 'SM' 
									AND SHI.POFulfillmentId = POFF.Id AND SHI.[Status] = 'active'
							)
				)
				OR EXISTS (
					SELECT 1
					FROM POFulfillmentAllocatedOrders POFFAO WITH(NOLOCK)
					INNER JOIN POFulfillments POFF WITH(NOLOCK) ON POFF.Id = POFFAO.POFulfillmentId AND POFF.[Status] = 10 AND POFF.Stage >= 20 AND POFF.FulfilledFromPOType = 20
