...
IF EXISTS(SELECT * FROM sys.views WHERE NAME = 'vw_PurchaseOrderList_CustomsCleared_ExternalUsers')
BEGIN
	 DROP VIEW vw_PurchaseOrderList_CustomsCleared_ExternalUsers;
END
GO

CREATE VIEW [dbo].[vw_PurchaseOrderList_CustomsCleared_ExternalUsers]
AS
	...
	SELECT	
		...
		CROSS APPLY (
				...
				CROSS APPLY (
							...
							WHERE ACT.ActivityCode = '2041' AND GACT.GlobalId = CONCAT('SHI_', SHI.Id)
						)ACT
