...
IF EXISTS(SELECT * FROM sys.views WHERE NAME = 'vw_ShipmentList_MovementShipmentVolumeInThisWeek_InternalUsers')
BEGIN
	 DROP VIEW vw_ShipmentList_MovementShipmentVolumeInThisWeek_InternalUsers;
END
GO

CREATE VIEW [dbo].[vw_ShipmentList_MovementShipmentVolumeInThisWeek_InternalUsers]
AS
	SELECT 
			SHI.Id,
			SHI.ShipmentNo,
