...
IF EXISTS(SELECT * FROM sys.views WHERE NAME = 'vw_PurchaseOrderList_ManagedToDate_InternalUsers')
BEGIN
	 DROP VIEW vw_PurchaseOrderList_ManagedToDate_InternalUsers;
END
GO

CREATE VIEW [dbo].[vw_PurchaseOrderList_ManagedToDate_InternalUsers]
AS
...
(
				...
				FROM Shipments SHI WITH (NOLOCK) INNER JOIN ShipmentCTE CTE on SHI.Id = CTE.Id
					INNER JOIN POFulfillmentOrders POD WITH (NOLOCK) ON SHI.POFulfillmentId = POD.POFulfillmentId AND POD.[Status] = 1
				GROUP BY POD.PurchaseOrderId
				HAVING POD.PurchaseOrderId = PO.Id
