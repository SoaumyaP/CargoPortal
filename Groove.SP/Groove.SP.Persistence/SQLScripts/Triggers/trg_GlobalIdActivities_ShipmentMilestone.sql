...
IF EXISTS (SELECT * FROM sys.objects WHERE [name] = N'trg_GlobalIdActivities_ShipmentMilestone' AND [type] = 'TR')
BEGIN
	DROP TRIGGER [dbo].trg_GlobalIdActivities_ShipmentMilestone;
END
GO
...
-- =============================================
CREATE TRIGGER  [dbo].[trg_GlobalIdActivities_ShipmentMilestone] 
   ON  [dbo].[GlobalIdActivities] 
...
BEGIN
	...
	SELECT SHM.Id
	FROM Shipments SHM
	WHERE CONCAT('SHI_', SHM.Id) IN (SELECT GlobalId FROM inserted UNION ALL SELECT GlobalId FROM deleted)
	...
	WHERE EXISTS (
		SELECT 1
		FROM inserted INS
		INNER JOIN Itineraries ITI ON CONCAT('FSC_', ITI.ScheduleId) = INS.GlobalId AND (ITI.ModeOfTransport = 'sea' OR ITI.ModeOfTransport = 'air')
		INNER JOIN ConsignmentItineraries CI ON CI.ItineraryId = ITI.Id
		WHERE SHM.Id = CI.ShipmentId 
	)
	...
	WHERE EXISTS (
		SELECT 1
		FROM deleted DEL
		INNER JOIN Itineraries ITI ON CONCAT('FSC_', ITI.ScheduleId) = DEL.GlobalId AND (ITI.ModeOfTransport = 'sea' OR ITI.ModeOfTransport = 'air')
		INNER JOIN ConsignmentItineraries CI ON CI.ItineraryId = ITI.Id
		WHERE SHM.Id = CI.ShipmentId 
	)
	...
	CROSS APPLY
		(
			...
			FROM (
					...
					INNER JOIN Itineraries ITI ON CONCAT('FSC_', ITI.ScheduleId) = GA.GlobalId 
					INNER JOIN ConsignmentItineraries CI ON CI.ItineraryId = ITI.Id
