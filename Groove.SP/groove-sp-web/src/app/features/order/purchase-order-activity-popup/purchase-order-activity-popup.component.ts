...
import { DropDowns, UserContextService, FormComponent, StringHelper, EventLevelMapping } from 'src/app/core';
...
@Component({
    selector: 'app-purchase-order-activity-popup',
    templateUrl: './purchase-order-activity-popup.component.html',
    styleUrls: ['./purchase-order-activity-popup.component.scss']
})
export class POActivityFormComponent extends FormComponent implements OnChanges {
    ...
    ngOnChanges(value: SimpleChanges) {
        if (value.allEventOptions && this.allEventOptions) {
            ...
            if (this.isEditModeLocal && value.model.currentValue?.id) {
                ...
                if (selectedEvent) {
                    this.disableEventSelection = selectedEvent.activityTypeLevel !== EventLevelMapping.PurchaseOrder || !selectedEvent.status;
            ...
            if (!this.disableEventSelection) {
                this.originFilteredEventOptions = this.originFilteredEventOptions.filter(event => event.activityTypeLevel === EventLevelMapping.PurchaseOrder && event.status);
