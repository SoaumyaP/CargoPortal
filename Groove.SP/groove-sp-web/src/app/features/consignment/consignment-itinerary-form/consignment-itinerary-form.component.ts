...
import { ScheduleFilterSet } from '../../shipment/consignment-itinerary-form/consignment-itinerary-form.component';
...
/**
 * It belongs to Consignment module
 */
@Component({
...
export class ConsignmentItineraryFormComponent extends FormComponent implements OnChanges {
    @Input() public itineraryFormOpened: boolean = false;
    @Input() public model: any;
    @Input() public shipmentNo: string;
    @Input() public consignmentId: any;
    @Input() public consignment: any;
    ...
    constructor(protected route: ActivatedRoute,
        ...
        public consignmentItineraryFormService: ConsignmentItineraryFormService,
    ...
    ngOnChanges(value: any): void {
        if (this.isAddModeLocal) {
            if (value.itineraryFormOpened?.currentValue) {
                this.model.modeOfTransport = this.consignment?.modeOfTransport;
        ...
        if (this.isEditModeLocal || (this.isViewModeLocal && this.model.departureDates)) {
            this.filteredCarrierOptions = this.allCarrierOptions.filter(c =>
                c.modeOfTransport.toLowerCase() === this.model.modeOfTransport?.toLowerCase());
            this.filteredPortOptions = this.allPortOptions;
            this.updateLiteralLabels(this.model.modeOfTransport);
    ...
    setPortByDefault() {
        ...
        const shipFromPort = this.allPortOptions.find(x => x.locationDescription.toUpperCase() === this.consignment?.shipFrom?.toUpperCase());
        const shipToPort = this.allPortOptions.find(x => x.locationDescription.toUpperCase() === this.consignment?.shipTo?.toUpperCase());
        if (this.itineraries?.length > 0) {
            const lastItinerary = this.itineraries[this.itineraries.length - 1];
    ...
    /** To set default departure date. It only works for sea mode. */
    setDepartureByDefault() {
        if (this.isEditModeLocal || this.model?.modeOfTransport.toLowerCase() !== ModeOfTransportType.Sea.toLowerCase()) {
            return;
        }
        if (this.itineraries?.length > 0) {
            const lastItinerary = this.itineraries[this.itineraries.length - 1];
            ...
            this.model.departureDates = new Date(this.consignment?.shipFromETDDate);
        }
    }

    onSequenceValueChanged() {
        const sequences = this.itineraries?.filter(x => x.id !== this.model.id)
            .map(x => x.sequence);
    ...
    getSchedules() {
        ...
        const selectedCMCarrierCode = this.consignment?.shipment?.contractMaster?.carrierCode;
        ...
        this.service.getSchedules(filterSetObj.buildToQueryParams).subscribe(
            schedules => {
                ...
                if (!this.isAddModeLocal && this.isSeaOrAir) {
    ...
    onAddClick() {
        ...
        if (!this.mainForm.valid || this.isDisableFormButton) {
            return;
        }
    ...
    private generateVesselFlight() {
        ...
        if (this.model.modeOfTransport.toLowerCase() === ModeOfTransportType.Air.toLowerCase()) {
            if (StringHelper.isNullOrWhiteSpace(this.model.flightNumber)) {
                return;
            }

            this.model.vesselFlight = this.model.flightNumber;
        }
