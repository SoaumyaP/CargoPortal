...
@Component({
    selector: 'app-po-fulfillment-contact',
    templateUrl: './po-fulfillment-contact.component.html',
    styleUrls: ['./po-fulfillment-contact.component.scss'],
    viewProviders: [{ provide: ControlContainer, useExisting: NgForm }]
})
export class POFulfillmentContactComponent implements OnChanges, OnInit, OnDestroy {
    @Input('data')
    contactList: any[];
    @Input()
    ...
    _registerShipFromChanged() {
        const sub = this.parentIntegration$.pipe(
            filter((eventContent: IntegrationData) =>
                eventContent.name === '[po-fulfillment-form]shipFromPortValueChanged'
            )).subscribe((eventContent: IntegrationData) => {

                // Need setTimeout 1s for below function to run after both ShipFrom changed and setOrgToContact function in app-po-fulfillment-form has finished
                setTimeout(() => {
    ...
    _registerShipToChanged() {
        const sub = this.parentIntegration$.pipe(
            filter((eventContent: IntegrationData) =>
                ...
                // Need setTimeout 1s for below function to run after both ShipTo changed and setOrgToContact function in app-po-fulfillment-form has finished
                setTimeout(() => {
                    if (this.agentAssignmentMode === AgentAssignmentMode.Default) {
                        const originDestinationContacts = this.contactList.filter(c => c.organizationRole === OrganizationNameRole.OriginAgent || c.organizationRole === OrganizationNameRole.DestinationAgent);
                        this.defaultContacts = cloneDeep(originDestinationContacts);
    ...
    /**
     * To set default values for Origin & Destination agent contacts
     */
    onSetDefaultAgent() {
