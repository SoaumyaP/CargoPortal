...
import { POFulfillmentFormService } from "./po-fulfillment-form.service";
...
import {
    ...
    AgentType,
    EquipmentType,
    PurchaseOrderAdhocChangePriority, POType, ModeOfTransportType, EntityType, AgentAssignmentMode, Roles, VerificationSetting, RoleSequence, AgentAssignmentMethodType, OrderFulfillmentPolicy, ViewSettingModuleIdType, FormModeType} from "src/app/core/models/enums/enums";
...
import { POFulfillmentTabModel } from "../po-fulfillment-models/po-fulfullment-tab.model";
import { FormHelper } from "src/app/core/helpers/form.helper";

@Component({
    selector: "app-po-fulfillment-form",
    templateUrl: "./po-fulfillment-form.component.html",
    styleUrls: ["./po-fulfillment-form.component.scss"]
})
export class POFulfillmentFormComponent extends FormComponent implements OnDestroy {
    ...
    @ViewChild(POFulfillmentGeneralInfoComponent, { static: false })
    poFulfillmentGeneralInfoControl: POFulfillmentGeneralInfoComponent;

    @ViewChild(POFulfillmentLoadInfoComponent, { static: false })
    poFulfillmentLoadInfoComponent: POFulfillmentLoadInfoComponent;

    @ViewChild(POFulfillmentCustomerComponent, { static: false })
    poFulfillmentCustomerInfoComponent: POFulfillmentCustomerComponent;

    @ViewChild(POFulfillmentContactComponent, { static: false })
    poFulfillmentContactComponent: POFulfillmentContactComponent;

    @ViewChild(POFulfillmentLoadDetailComponent, { static: false })
    poFulfillmentLoadDetailComponent: POFulfillmentLoadDetailComponent;
    ...
    @ViewChild('shipment', { static: false })
    shipmentElement: ElementRef;

    @ViewChild('loadDetails', { static: false })
    loadDetailsElement: ElementRef;
    ...
    isReadyForAmendBooking: boolean = true;
    ...
    bookingValidationErrors = {
        ...
        load: null,
        /**To contain validation message for shipment section.
         * Pls use getErrorMessage('shipment) to get error message with full top at the end.*/
        shipment: null,
        /**To contain validation message for load detail section.
         * Pls use getErrorMessage('loadDetail) to get error message with full top at the end.*/
        loadDetails: null,
    ...
    constructor(
        ...
        this._registerModeOfTransportChangedHandler();
    }
    ...
    _registerShipFromPortChangedHandler() {
        const sub = this.integration$.pipe(
            filter((eventContent: IntegrationData) =>
                ...
                // Fire another event to update Original Agent on contact list
                if (eventContent.name === '[po-fulfillment-general-info]shipFromPortValueChanged') {
                const emitValue: IntegrationData = {
                    name: '[po-fulfillment-form]shipFromPortValueChanged',
                    description: 'On POFulfillmentFormComponent, after done handling event [po-fulfillment-general-info]shipFromPortValueChanged, it fires another event'
                };
    ...
    onInitDataLoaded(data): void {
        ...
        //set tab display
        this.setTabDisplay('load', !StringHelper.caseIgnoredCompare(this.model?.modeOfTransport, ModeOfTransportType.Air));
        this.setTabDisplay('shipment', this.model.stage >= POFulfillmentStageType.ForwarderBookingRequest);
        this.setTabDisplay('loadDetails', this.isShowLoadDetailsTab);
    ...
    async onSubmit() {
        ...
        if (this.isReloadMode) {
            let isValid = this.validateLoadDetailTab('popup');
            if (!isValid) {
                this.isReadyForSubmit = true;
                return;
            }

            isValid = this.validatePOFFBeforePlanToShip();
            if (!isValid) {
                this.isReadyForSubmit = true;
                return;
            }
            ...
            confirmDlg.result.subscribe((result: any) => {
                if (result.value) {
                    ...
                    this.service.reload(this.modelId, tempModel).subscribe(
                        data => {
                            this._gaService.emitAction('Reload', appConstants.GAEventCategory.POBooking);
    ...
    private validatePOFFBeforeSaving(): boolean {
        ...
        // validate tab load details
        if (!this._validateInputsInTab('loadDetails')) {
            return false;
        }

        const isValidLoadDetailsTab = this.validateLoadDetailTab('scroll');
        if (!isValidLoadDetailsTab) {
            return false;
        }
    ...
    validateLoadDetailTab(errorAlertType: 'scroll' | 'popup' | null = null): boolean {
        if (this.model.stage < POFulfillmentStageType.ForwarderBookingConfirmed) {
            return true;
        }
        if (this.model.movementType !== appConstants.MovementTypes.CYUnderscoreCY) {
            return true;
        }

        this.bookingValidationErrors.loadDetails = null;
        const loadDetailValidationResult =  this.poFulfillmentLoadDetailComponent.validateTab();

        if (loadDetailValidationResult?.status === false) {
            if (errorAlertType === 'scroll')
            {
                this.bookingValidationErrors.loadDetails = loadDetailValidationResult.message;
                this.scrollToSection("loadDetails");
            }
            else if(errorAlertType === 'popup') {
                this.notification.showErrorPopup(loadDetailValidationResult.message, 'label.poFulfillment');
            }
            return false;
        }
    ...
    get hiddenBtnAmend() {
        if (this.model.stage !== POFulfillmentStageType.ForwarderBookingRequest &&
            !(this.model.stage ===  POFulfillmentStageType.Draft && this.isPendingStatus())
            ) {
            return true;
        }
    ...
    get hiddenBtnDispatch() {
        ...
        if (this.model.stage !== POFulfillmentStageType.ShipmentDispatch) {
            return true;
        }
        if (!this.currentUser.isInternal) {
            if (this.currentUser.role.id !== Roles.Shipper && this.currentUser.role.id !== Roles.Factory) {
    ...
    get isShowCargoDetailsTab() {
        return false;
    }

    get isShowShipmentTab() {
        return this.model.stage >= POFulfillmentStageType.ForwarderBookingRequest;
    }
    ...
    get isItineraryTabEditable() {
        if (this.isViewMode) {
            return false;
        }

        if (this.model.isForwarderBookingItineraryReady) {
            return false;
        }
        ...
        if (this.currentUser.isInternal) {
            return (
                this.model.stage ===
                POFulfillmentStageType.ForwarderBookingRequest
            );
        }
        return false;
    }
    ...
    onCargoDetailItemAdded(cargoDetail) {
        this.model.cargoDetails.push(cargoDetail);
        this.calTotalUnitQuantityOfCargoDetails();
    }

    onCargoDetailItemEdited(cargoDetail) {
        this.model.cargoDetails.forEach((el, i) => {
            if (
                el.id === cargoDetail.id &&
                el.lineOrder === cargoDetail.lineOrder
            ) {
                this.model.cargoDetails[i] = cargoDetail;
            }
        });
        this.calTotalUnitQuantityOfCargoDetails();
    }

    onCargoDetailItemDeleted(lineOrder) {
        this.model.cargoDetails = this.model.cargoDetails.filter(el => {
    ...
    onBookButtonClick() {
        ...
        mandatoryFields.forEach(f => {
            if (StringHelper.isNullOrEmpty(this.model[f]) || this.model[f] === 0) {
                ...
                if (StringHelper.isNullOrEmpty(currentValue)) {
                    this.bookingValidationErrors.general = this.translateService.instant(`label.${labelMapped}`);
                } else {
        ...
        if (!StringHelper.isNullOrEmpty(this.bookingValidationErrors.general)) {
            const missingThings = this.bookingValidationErrors.general;
            this.bookingValidationErrors.general = this.translateService.instant('msg.missingThing', {thing: missingThings});
        ...
        requiredOrgRoles.forEach(organizationRole => {
            if (!this.model.contacts.find(c => c.organizationRole === organizationRole)) {
                ...
                if (StringHelper.isNullOrEmpty(currentValue)) {
                    this.bookingValidationErrors.contact = organizationRole;
                } else {
                    this.bookingValidationErrors.contact = currentValue + ', ' + organizationRole;
                }
                valid = false;
            }
        });
        ...
        if (!this.model.isNotifyPartyAsConsignee && !contactConsignee) {
            const currentValue = this.bookingValidationErrors.contact;
            if (StringHelper.isNullOrEmpty(currentValue)) {
                this.bookingValidationErrors.contact = OrganizationNameRole.Consignee.toString();
            } else {
        ...
        if (!StringHelper.isNullOrEmpty(this.bookingValidationErrors.contact)) {
            const missingThings = this.bookingValidationErrors.contact;
            this.bookingValidationErrors.contact += this.translateService.instant('msg.missingThing', {thing: missingThings});
        ...
        if (this.model.orders.length === 0) {
            // this.bookingValidationErrors.customerPOTab = 'There must be at least one record.';
        ...
        if (this.model.loads.length === 0) {
            // this.bookingValidationErrors.loadTab = 'There must be container load information.';
    ...
    private forceCreateBooking() {
        this.service.createPOFulfillmentBookingRequest(this.model.id).subscribe(
            (response) => {
                ...
                // Set tabs collection back to default before calling init.
                this.tabs.map(x => x.hidden = false);
    ...
    */
    ...
    onAmendPOFulfillmentClick() {
        ...
        confirmDlg.result.subscribe((result: any) => {
            if (result.value) {
                this.isReadyForAmendBooking = false;
                this.service.amendPOFulfillment(this.model.id).subscribe(
                    () => {
                        this._gaService.emitEvent('amend', appConstants.GAEventCategory.POBooking, 'Amend');
                        this.notification.showSuccessPopup(
                            "msg.amendPOFulfillmentSuccessfully",
                            "label.poFulfillment"
                        );

                        if (this.model.orderFulfillmentPolicy === OrderFulfillmentPolicy.AllowMissingPO) {
                            this.router.navigate([`/missing-po-fulfillments/view/${this.model.id}`], {
                                queryParams: {
                        ...
                        this.isReadyForAmendBooking = true;

                        // Need to reset the shared customer PO list to prevent out of date cache
                        this.service.resetCustomerPOs();
    ...
    onPlanToShipClick() {
        // Tab Cargo Details is always hidden from now as business

        // Proceed only if tab Load Details is showing
        if (this.isShowLoadDetailsTab) {
            let isValid = this.validatePOFFBeforePlanToShip();
            if (isValid) {
                this.executePlanToShip();
            }
        }
    }

    private validatePOFFBeforePlanToShip(): boolean {
        const isLoadEmpty =
                this.model.loads &&
        ...
        if (!this.validateQtyLoadDetails()) {
            this.notification.showInfoDialog(
                'validation.loadDetailNotEqualQty',
                'label.poFulfillment',
                false,
                true,
                475
    ...
    executePlanToShip() {
        this.service.planToShip(this.model.id).subscribe(
            () => {
                this._gaService.emitAction('Plan to ship', appConstants.GAEventCategory.POBooking);
                this.notification.showSuccessPopup(
    ...
    onDispatchClick() {
        const confirmDlg = this.notification.showConfirmationDialog(
            "msg.confirmDispatchPOFF",
            "label.poFulfillment"
        );

        confirmDlg.result.subscribe((result: any) => {
            if (result.value) {
                this.service.dispatch(this.model.id).subscribe(
                    () => {
                        this._gaService.emitAction('Dispatch', appConstants.GAEventCategory.POBooking);
                        this.notification.showSuccessPopup(
                            "save.sucessNotification",
                            "label.poFulfillment"
                        );
                        if (!this.stringHelper.isNullOrEmpty(this.bookingPOAdhocChange) && this.bookingPOAdhocChange.priority === this.purchaseOrderAdhocChangePriority.Level3) {
                            this.bookingPOAdhocChange = null;
                        }
                        this.ngOnInit();
                    },
    ...
    private validateQtyLoadDetails(): boolean {
        if (
            this.model.stage >= POFulfillmentStageType.ForwarderBookingConfirmed
        ) {
            const customerPOTotalFulfillmentUnitQty = this.calTotalFulfillmentUnitQtyOfCustomerPO();
            const loadsTotalPlannedPackageQty = this.calTotalPlannedPackageQtyOfLoads();

            const loadDetailsTotalUnitQty = this.calTotalUnitQtyOfLoadDetails();
            const loadDetailsTotalPackageQty = this.calTotalPackageQtyOfLoadDetails();

            // Do not validate Quantity if the user has not added Cargo Detail and Load Detail yet.
            if (!this.hasLoadDetail) {
                return true;
            }

            return (
                loadDetailsTotalUnitQty === customerPOTotalFulfillmentUnitQty &&
                loadDetailsTotalPackageQty === loadsTotalPlannedPackageQty
            );
        } else {
            return true;
        }
    ...
    private calTotalUnitQuantityOfCargoDetails() {
        let sum = 0;
        this.model.cargoDetails.forEach(el => {
            sum += el.unitQuantity;
            this.hasCargoDetail = true;
        });

        return sum;
    }

    private calTotalFulfillmentUnitQtyOfCustomerPO() {
        let sum = 0;
        this.model.orders.forEach(el => {
            sum += el.fulfillmentUnitQty;
        });

        return sum;
    }

    private calTotalPlannedPackageQtyOfLoads() {
        let sum = 0;
        this.model.loads.forEach(el => {
            sum += el.plannedPackageQuantity;
        });

        return sum;
    }

    private calTotalUnitQtyOfLoadDetails() {
        let sum = 0;
        this.model.loads.forEach(el => {
            sum += el.subtotalUnitQuantity;
            this.hasLoadDetail = el.details && el.details.length > 0;
        });

        return sum;
    }

    private calTotalPackageQtyOfLoadDetails() {
        let sum = 0;
        this.model.loads.forEach(el => {
            sum += el.subtotalPackageQuantity;
        });
    ...
    onItineraryConfirmed() {
        // check itinerary as only clicking on confirm button
        if (this.isItineraryTabEditable && this.model.itineraries.length === 0) {
            this.notification.showErrorPopup(
                "msg.savePOFulfillmentItineraryFailed",
                "label.poFulfillment"
            );
            return;
        }
        this.model.isForwarderBookingItineraryReady = true;
        this.onSubmit();
    }
    ...
    private linkTab(tab : POFulfillmentTabModel) {
        switch (tab.sectionId) {
            ...
            case 'load':
                ...
                break;
            case 'shipment':
                tab.sectionElementRef = this.shipmentElement;
                break;
            case 'loadDetails':
    ...
    private navigate(modeType: FormModeType, ...additionalParams: {name: string, value: string}[]) : void {
        ...
        this.router.navigate([`/po-fulfillments/${modeType}/${this.model.id}`], {
            queryParams: params
        });
    }
