...
import { faPlus, faEllipsisV, faPencilAlt, faTrashAlt, faInfoCircle, faTrash } from '@fortawesome/free-solid-svg-icons';
...
import { POCustomerFormComponent } from '../po-customer-form/po-customer-form.component';
import { ActivatedRoute, ActivatedRouteSnapshot } from '@angular/router';
import { OrganizationNameRole, LogisticsService, ValidationDataType, Roles, POFulfillmentStageType, ModeOfTransportType, POType, PackageUOMType, AgentAssignmentMode, RoleSequence, ViewSettingModuleIdType } from 'src/app/core/models/enums/enums';
...
@Component({
    selector: 'app-po-fulfillment-customer',
    templateUrl: './po-fulfillment-customer.component.html',
    styleUrls: ['./po-fulfillment-customer.component.scss'],
...
export class POFulfillmentCustomerComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    @ViewChild(POCustomerFormComponent, { static: false }) public POCustomerFormElement: POCustomerFormComponent;
    ...
    private bindBookingByComplianceSettings() {
        const firstPO = this.selectCustomerPOSelected_InOrders_ByIndex(0);
        const emitValue = {
    ...
    customerAddHandler(modelPopup) {
        ...
        if (this.model.orders.length === 1) {
            ...
            if (this.model.modeOfTransport === ModeOfTransportType.Sea) {
                const containerType = this.availablePOsList.find(x => x.id === this.model.orders[0].purchaseOrderId).containerType;
    ...
    _validateCustomerPOFromComplianceSettings() {
        ...
        if (isAllowPartialShipment && this.poType !== POType.Blanket) {
            ...
            const selectedPONumbers = this.model.orders.map(item => item.customerPONumber)
                .filter((value, index, self) => self.indexOf(value) === index);
            const customerPOs = this.service.currentCustomerPOs();
            for (let i = 0; i < selectedPONumbers.length; i++) {
                const lineItems = customerPOs.find(x => x.poNumber === selectedPONumbers[i]).lineItems;
                for (let j = 0; j < lineItems.length; j++) {
                    if (!this.model.orders.some(x => x.customerPONumber === selectedPONumbers[i]
    ...
    validateTab() {
        ...
        const loads = this.model.loads;

        if ((StringHelper.isNullOrEmpty(customerPOs) || customerPOs.length === 0) && (!StringHelper.isNullOrEmpty(loads) && loads.length > 0)) {
            const result = new ValidationData(ValidationDataType.Business, true);
            result.status = false;
