...
import { DropDowns, UserContextService, FormComponent, StringHelper, OrderType, ActivityType, EventLevelMapping } from 'src/app/core';
...
@Component({
    selector: 'app-shipment-activity-form',
    templateUrl: './shipment-activity-form.component.html',
    styleUrls: ['./shipment-activity-form.component.scss']
})
export class ShipmentActivityFormComponent extends FormComponent implements OnChanges {
    ...
    ngOnChanges(value: SimpleChanges) {
        if (value.allEventOptions && this.allEventOptions) {
            ...
            if (this.isEditModeLocal && value.model.currentValue?.id) {
                ...
                if (selectedEvent) {
                    this.disableEventSelection = selectedEvent.activityTypeLevel !== EventLevelMapping.Shipment || !selectedEvent.status;
            ...
            if (!this.disableEventSelection) {
                this.originFilteredEventOptions = this.originFilteredEventOptions.filter(event => event.activityTypeLevel === EventLevelMapping.Shipment && event.status);
