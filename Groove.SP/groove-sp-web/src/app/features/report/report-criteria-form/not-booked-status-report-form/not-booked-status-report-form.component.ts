...
@Component({
  selector: 'app-not-booked-status-report-form',
  templateUrl: './not-booked-status-report-form.component.html',
  styleUrls: ['./not-booked-status-report-form.component.scss']
})
export class NotBookedStatusReportFormComponent extends ReportFormBase implements OnInit, OnDestroy {
  ...
  onFormInit() {
    this.mainForm = new FormGroup({
      ...
      includeDraftBooking: new FormControl(true)
    });
  }
  ...
  onExportClick() {
    ...
    const filterDsModel = new NotBookedStatusReportFilterData(
    ...
    this.reportCriteriaFormService.exportXlsx(
      this.selectedReportId, 'Not Booked Status Report.xlsx',
      filterDsModel.buildToQueryParams).subscribe(response => {
        if (!response) {
...
export class NotBookedStatusReportFilterData extends QueryModel {
